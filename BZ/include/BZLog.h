...
#define LogWrap(str, ...) (															\
		(BoZhi::GetInstance()->GetCallbacks()->UtilsFuncs.Log) ?					\
		BoZhi::GetInstance()->GetCallbacks()->UtilsFuncs.Log(str, ##__VA_ARGS__) :	\
		LogBase(str, ##__VA_ARGS__))

#define BZLogError(on, str, ...)	((on) ? LogWrap(LOG_MODULE "E" LOG_FORMAT str LOG_FMT_PARAM, ##__VA_ARGS__) : (0))
#define BZLogWarn(on, str, ...)		((on) ? LogWrap(LOG_MODULE "W" LOG_FORMAT str LOG_FMT_PARAM, ##__VA_ARGS__) : (0))
#define BZLogInfo(on, str, ...)		((on) ? LogWrap(LOG_MODULE "I" LOG_FORMAT str LOG_FMT_PARAM, ##__VA_ARGS__) : (0))
#define BZLogDebug(on, str, ...)	((on) ? LogWrap(LOG_MODULE "D" LOG_FORMAT str LOG_FMT_PARAM, ##__VA_ARGS__) : (0))
...
#define BLOGDP(str, ...) ((DBG_LEVEL & DBG_PROC_MASK) ? BLOGD(str, ##__VA_ARGS__) : (0))
